...
function mergeProfiles(what, userObj, updateProf, updateCreds, callback) {
    ...
    if(updateProf != undefined) {
        finalProfile = {};
        for(var prop in userObj.profile) finalProfile[prop] = userObj.profile[prop];
        for(var prop in updateProf) finalProfile[prop] = updateProf[prop];
    }
    if(updateCreds != undefined) {
        finalCreds = {};
        for(var prop in userObj.credentials) finalCreds[prop] = userObj.profile[prop];
        for(var prop in updateCreds) finalCreds[prop] = updateCreds[prop];
    }
